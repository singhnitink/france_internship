...
for file in files:
    ...
    lines=data.shape[0]
    nres=data.shape[1] #number of residues in peptide chain
    print(f'Number of residues is {nres-1}')
    ...
    plt.plot(np.arange(1,nres,1),final_result, linestyle='-', marker=marker[file_counter], color=colors[file_counter],label=label,linewidth=3,markersize=15)
    #plt.bar(np.arange(1,nres,1)+width,final_result, color=colors[file_counter],width = 0.33,label=label)
    file_counter+=1
    width=width+0.30
legend = plt.legend(numpoints = 1, loc='best',frameon=False, fontsize=24)
#legend.get_frame().set_facecolor('none')
plt.yticks(np.arange(0,1.2,0.1),fontsize=18,weight='bold')
plt.xticks(np.arange(0,nres,1),fontsize=18,weight='bold')
plt.ylim(0.0,1.10)
...
plt.title(r'XXXX (helix) Membrane (All atom)',fontsize=24,weight='bold')
ax = plt.gca()
...
plotfile='XXXX_helix_membraneAA.png'
